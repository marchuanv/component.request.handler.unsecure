...
module.exports = { 
    ...
    handle: (context, options) => {
        delegate.register("component.request.handler.secure", async ( { headers, data, privateKey, hashedPassphrase, privatePort }) => {
            if (options.privatePort === privatePort){
                if (!options.hashedPassphrase || !options.hashedPassphraseSalt) { 
                    ...
                    return await delegate.call({context}, { headers, data });
                }
                ...
                if (session){
                    ...
                    let results = await delegate.call({context}, { headers, data });
                    if (!results){
